...
`useInfiniteRetrieve` allows to perform [infinite pagination](/pagination#infinite)
by offering a way to configure how it returns values based on the current
response and state.
...
can create a custom hook and use the `UseInfiniteRetrieveFactory` type:
